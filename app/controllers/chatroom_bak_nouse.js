	...
	for (var i=0; i < data.length; i++) {
		...
		*/
		if(data[i].sender_id){
			...
			}else if(data[i].format == "photo"){
				var img_photo = $.UI.create("ImageView", {classes:['wfill','hsize','padding'], image:newText});
				view_text_container.add(label_name);
				view_text_container.add(img_photo);
			}else{
				view_text_container.add(label_name);
				view_text_container.add(label_message);
			}
...
function getLatestData(){
	...
	//model.getData(true,"","", last_update, args.u_id); 
...
function scrollChecker(e){
	...
	var nearEnd = ($.msg_container.rect.height-$.chatroom.rect.height) - 200;
